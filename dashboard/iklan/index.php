...
$totalIklan = (int)($countResult->fetch_assoc()['total'] ?? 0);
$totalPages = $totalIklan > 0 ? (int)ceil($totalIklan / $perPage) : 1;
...
$iklan = [];
$result = $db->query("
    SELECT i.*, l.action, l.description, l.created_at as log_created_at
    FROM iklan i
            ...
            <div class="mb-6 sm:mb-8 relative overflow-hidden rounded-2xl bg-white/90 backdrop-blur-sm border border-slate-200/60 shadow-lg shadow-slate-200/50">
                ...
                <div class="relative px-5 sm:px-6 py-5 sm:py-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5">
                    ...
                    <div class="flex items-center gap-3 flex-wrap sm:flex-nowrap">
                        <div class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-purple-100/80 border border-purple-200/60">
                            <div class="w-2 h-2 rounded-full bg-purple-500 animate-pulse"></div>
                            <span class="text-sm font-semibold text-purple-700"><?php echo number_format($totalIklan); ?></span>
                            <span class="text-purple-600 text-sm">permohonan</span>
                        </div>
                        <?php if ($totalIklan > 0): ?>
                            <form action="/dashboard/iklan/process.php" method="POST" class="inline"
                                onsubmit="return confirm('Hapus SEMUA <?php echo number_format($totalIklan); ?> data iklan?\n\nTindakan ini tidak dapat dibatalkan.');">
                    ...
                    <table class="w-full">
                        <thead class="bg-slate-50/50">
                            <tr>
                                ...
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Nama / Kontak</th>
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Email</th>
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider hidden md:table-cell">Telepon</th>
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Jenis Iklan</th>
                                <th class="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider hidden lg:table-cell">Pesan</th>
                                ...
                                <tr>
                                    <td colspan="8" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-slate-500 font-medium">Tidak ada data iklan</p>
                                            <p class="text-slate-400 text-sm mt-1">Belum ada permohonan iklan</p>
                                        </div>
                                    </td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($iklan as $index => $item): ?>
                                    <tr class="hover:bg-slate-50/50 transition-colors duration-150 <?php echo $index % 2 === 0 ? 'bg-white/50' : ''; ?>">
                                        ...
                                        <td class="px-4 sm:px-6 py-4">
                                            <div class="text-sm text-slate-700 truncate max-w-xs" title="<?php echo htmlspecialchars($item['email']); ?>">
                                        ...
                                        <td class="px-4 sm:px-6 py-4">
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold shadow-sm bg-purple-100 text-purple-700 border border-purple-200">
                                                <?php echo htmlspecialchars($item['jenis']); ?>
                                            </span>
                                        </td>
                                        <td class="px-4 sm:px-6 py-4 hidden lg:table-cell">
                                            <div class="text-sm text-slate-600 max-w-xs truncate" title="<?php echo htmlspecialchars($item['pesan']); ?>">
                                        ...
                                        <td class="px-4 sm:px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                ...
                                                $emailSubject = urlencode('Balasan: Permohonan Iklan - ' . htmlspecialchars($item['jenis']));
                                                $emailBody = urlencode('Halo ' . htmlspecialchars($item['nama']) . ",\n\nTerima kasih atas permohonan iklan Anda.\n\nKami akan segera menghubungi Anda untuk membahas lebih lanjut.\n\nSalam,\nTim Blog News");
                                                $emailLink = 'mailto:' . htmlspecialchars($item['email']) . '?subject=' . $emailSubject . '&body=' . $emailBody;

                                                // Format WhatsApp (hapus karakter non-digit, pastikan dimulai dengan 62 untuk Indonesia)
                                                $telepon = preg_replace('/[^0-9]/', '', $item['telepon']);
                                                if (substr($telepon, 0, 1) === '0') {
                                                    $telepon = '62' . substr($telepon, 1);
                                                } elseif (substr($telepon, 0, 2) !== '62') {
                                                    $telepon = '62' . $telepon;
                                                }
                                                $waMessage = urlencode('Halo ' . htmlspecialchars($item['nama']) . ', terima kasih atas permohonan iklan Anda. Kami akan segera menghubungi Anda untuk membahas lebih lanjut.');
                                                $waLink = 'https://wa.me/' . $telepon . '?text=' . $waMessage;
                                                ?>
                                                ...
                                                <form action="/dashboard/iklan/process.php" method="POST" class="inline"
                                                    onsubmit="return confirm('Hapus data iklan <?php echo htmlspecialchars($item['nama']); ?>?');">
                ...
                <?php if ($totalPages > 1): ?>
                    <div class="px-4 sm:px-6 py-4 border-t border-slate-200/50 bg-slate-50/30 flex flex-wrap items-center justify-between gap-2">
                        <p class="text-sm text-slate-600">
                            ...
                            <span class="text-slate-400">(<?php echo number_format($totalIklan); ?> total)</span>
                        </p>
                        <div class="flex items-center gap-2">
                            <?php if ($page > 1): ?>
